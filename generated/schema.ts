...
export class FeedEvent extends Entity {
  constructor(id: Bytes) {
    super();
    this.set("id", Value.fromBytes(id));
  }
  ...
  save(): void {
    let id = this.get("id");
    assert(id != null, "Cannot save FeedEvent entity without an ID");
    if (id) {
      assert(
        id.kind == ValueKind.BYTES,
        `Entities of type FeedEvent must have an ID of type Bytes but the id '${id.displayData()}' is of type ${id.displayKind()}`,
      );
      store.set("FeedEvent", id.toBytes().toHexString(), this);
    }
  }

  static loadInBlock(id: Bytes): FeedEvent | null {
    return changetype<FeedEvent | null>(
      store.get_in_block("FeedEvent", id.toHexString()),
    );
  }

  static load(id: Bytes): FeedEvent | null {
    return changetype<FeedEvent | null>(
      store.get("FeedEvent", id.toHexString()),
    );
  }
